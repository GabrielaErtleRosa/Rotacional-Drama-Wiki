...
>>     );
>>
>>     // normaliza os paths dos filhos do sim atual, pra comparar depois
>>     const myChildrenPaths = new Set(
>>         (char.Children ?? [])
>>             .map(c => c?.path ?? c)
>>             .filter(Boolean)
>>     );
>>
>>     if (myFamilies.size > 0 || myChildrenPaths.size > 0) {
>>         const related = dv.pages('"Sims"')
>>             .where(s => {
>>                 if (s.file.path === char.file.path) return false;
>>
>>                 // condição 1: família em comum (tag)
>>                 const sharesFamily = (s.file.tags ?? []).some(t =>
>>                     myFamilies.has(t.toLowerCase())
>>                 );
>>
>>                 // condição 2: filho em comum (campo Children)
>>                 const theirChildrenPaths = (s.Children ?? [])
>>                     .map(c => c?.path ?? c)
>>                     .filter(Boolean);
>>
>>                 const sharesChild = theirChildrenPaths.some(p =>
>>                     myChildrenPaths.has(p)
>>                 );
>>
>>                 return sharesFamily || sharesChild;
>>             });
>>
>>         if (related.length > 0) {
...
>[!Tags]
>#Family/Goth #Family/KimLewis #Family/a #Family/Lee #Family/L  #Family/b #Family/v #Family/f